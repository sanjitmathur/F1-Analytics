...
function BracketSection({ title, drivers, teamColors, color }: {
  title: string;
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        {drivers.map(d => (
          <div key={d.driver_name} style={{
            ...
            }}>P{Math.round(d.predicted_position)}</span>
...
export default function QualifyingBracket({ results, teamColors }: Props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <BracketSection title="Q3 — Top 12" drivers={q3Drivers} teamColors={teamColors} color="var(--accent-green)" />
      {q2Eliminated.length > 0 && <BracketSection title="Q2 Eliminated" drivers={q2Eliminated} teamColors={teamColors} color="var(--accent-yellow)" />}
      {q1Eliminated.length > 0 && <BracketSection title="Q1 Eliminated" drivers={q1Eliminated} teamColors={teamColors} color="var(--f1-red)" />}
    </div>
  );
}
